...
namespace {
constexpr HiLogLabel LABEL = { LOG_CORE, SensorsLogDomain::SENSOR_TEST, "ClientInfoTest" };
...
{
    HiLog::Info(LABEL, "%{public}s begin", __func__);
    SensorState ret = clientInfo_.GetSensorState(ACC_SENSOR_ID);
...
{
    HiLog::Info(LABEL, "%{public}s begin", __func__);
    SensorState ret = clientInfo_.GetSensorState(MAG_SENSOR_ID);
    ASSERT_EQ(ret, SENSOR_ENABLED);
    HiLog::Info(LABEL, "%{public}s end", __func__);
}
...
HWTEST_F(ClientInfoTest, UpdateSensorChannel_003, TestSize.Level1)
{
    HiLog::Info(LABEL, "%{public}s begin", __func__);
    int32_t pid = 1000;
    sptr<SensorBasicDataChannel> sensorChannel = nullptr;
    auto ret = clientInfo_.UpdateSensorChannel(pid, sensorChannel);
    ASSERT_FALSE(ret);
    HiLog::Info(LABEL, "%{public}s end", __func__);
}
